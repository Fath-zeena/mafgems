...
const DynamicPendantViewer = dynamic(
  () => import("@/components/pendant-viewer").then((mod) => mod.PendantViewer),
  {
    ssr: false,
    ...
    <CustomizerProvider>
      <div className="flex flex-col md:flex-row h-screen w-screen bg-background text-foreground">
        <aside className="w-full md:w-80 lg:w-96 md:h-full p-4 overflow-y-auto border-b md:border-b-0 md:border-r border-border">
          <h1 className="text-2xl font-bold mb-4">Pendant Customizer</h1>
          <GemGallery />
        </aside>
        <main className="flex-1 h-full">
          <DynamicPendantViewer />
        </main>
      </div>
    </CustomizerProvider>
  );
